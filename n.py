...
# Add this near the other configuration constants (around line 50)
ATTACK_VIDEOS = [
...
async def show_running_attacks(update: Update, context: CallbackContext):
    ...
    # Send a random attack video
    attack_video = random.choice(ATTACK_VIDEOS)
    await context.bot.send_video(
        chat_id=update.effective_chat.id,
        video=attack_video,
        caption="🔥 CURRENTLY RUNNING ATTACKS 🔥"
    )
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    # Send random attack video
    attack_video = random.choice(ATTACK_VIDEOS)
    await context.bot.send_video(
        chat_id=update.effective_chat.id,
        video=attack_video,
        caption="🔥 ATTACK STARTED! 🔥"
    )
    ...
    try:
        ...
        if not active_attacks:
            ...
            finish_video = random.choice(FINISH_VIDEOS)
